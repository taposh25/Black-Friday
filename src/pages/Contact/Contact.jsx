...
const Contact = () => {
  ...
  const onSubmit = (data) => {
    // EmailJS এর মাধ্যমে ইমেইল পাঠানোর জন্য এই অবজেক্টটি তৈরি করা হয়েছে
    // আপনার EmailJS টেমপ্লেটের {{variable_name}} এর সাথে এগুলো মিল থাকতে হবে
    ...
    // আপনার EmailJS ড্যাশবোর্ড থেকে আইডিগুলো এখানে বসাবেন
    const serviceID = 'service_69sv9bp';
    const templateID = 'template_szyx10o';
    const publicKey = 'yl1_KegAysGU0e694';

    emailjs.send(serviceID, templateID, templateParams, publicKey)
      .then((response) => {
        console.log('SUCCESS!', response.status, response.text);
        alert("Message sent successfully!");
        reset(); // ইমেইল সফলভাবে চলে গেলে ফর্মটি খালি (Reset) হয়ে যাবে
      })
      .catch((err) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className={`w-full bg-white text-black p-4 focus:outline-none focus:ring-2 focus:ring-red-600 ${errors.lastName ? 'ring-2 ring-red-500' : ''}`}
                />
                {errors.lastName && <p className="text-red-500 text-xs uppercase">{errors.lastName.message}</p>}
              </div>
            </div>
